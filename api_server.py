    ...
    try:
        ...
        if "matches" in corrections_data:
            for match in corrections_data["matches"]:
                error_type = (
                    "grammar" if match["rule"]["issueType"] == "grammar" else "spelling"
                )
                ...
                corrections.append(
                    {
                        "original": match["context"]["text"][
                            match["context"]["offset"] : match["context"]["offset"]
        ...
        if has_last_paragraph:
            ...
            has_proposal_keywords = any(
                keyword in last_paragraph for keyword in proposal_keywords
            )
        ...
        if has_proposal_keywords:
            comp5_score_raw += 40
        comp5_score = add_variation(comp5_score_raw)
...
def save_results_api():
    ...
    try:
            ...
            corrections.append(
                {
                    ...
                    "context": {
                        "text": original_text[
                            max(0, corr["position"]["start"] - 10) : min(
                                len(original_text), corr["position"]["end"] + 10
                            )
                        ]
                    },
        ...
        analysis_details = {
            ...
            "teacher_grade": data["teacherInput"]["grade"]
            or str(data["essayAnalysis"]["score"]["total"] / 10),
